...
// Gleicher Endpoint wie bei AllTimesheetSubmissions –
// Backend liefert hier alle aktuell AUSZAHLBAREN Abrechnungen (Status 21 ohne 22)
...
// Aus Sicht GS: „Jetzt zu bezahlende“ = wir markieren hier als final/ausbezahlt
async function markAsPaid(id: number) {
  if (!confirm('Möchtest du diese Abrechnung als bezahlt kennzeichnen (finale Auszahlung)?')) return

  isProcessingId.value = id
  try {
    await axios.post(`${API_URL}/${id}/finalize`)
    // Nach Erfolg aus der Liste entfernen
    submissions.value = submissions.value.filter(s => s.AbrechnungID !== id)
  } catch (error: any) {
    alert('Fehler: ' + (error.response?.data?.message || 'Konnte nicht als bezahlt markiert werden.'))
  } finally {
  ...
  <div class="page">
    ...
    <v-card elevation="6" class="pa-4">
      <v-card-title class="pa-0 mb-4 d-flex align-center justify-space-between">
        <h3 class="ma-0">Abrechnungen zur Auszahlung (Geschäftsstelle)</h3>
        <v-btn
          size="small"
          variant="text"
          color="primary"
      ...
      <v-card-text class="pa-0">
        <div v-if="isLoading" class="placeholder">
          ...
          Lade abzuschließende Abrechnungen ...
        </div>
        ...
        <div v-else class="placeholder">
          <v-icon icon="mdi-cash-check" size="large" color="green" class="mb-2" />
          Keine Abrechnungen zur Auszahlung vorhanden.
        </div>
      </v-card-text>
    </v-card>
  </div>
